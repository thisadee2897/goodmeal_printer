...
import '../controllers/providers/document_bill_cash.dart';
import '../controllers/providers/document_bill_dt.dart';
...
class BillScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    Uint8List? filePdf = ref.watch(filePdfBillViewProvider);
    final hd = ref.watch(documentBillProvider);
    final dt = ref.watch(documentBillDTProvider);
    if(dt.hasError){
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          FilledButton.icon(
            onPressed: () async {
              var pdfFile = ref.read(filePdfBillProvider);
