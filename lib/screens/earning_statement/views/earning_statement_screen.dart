...
import '../controllers/providers/earning_statement_dt.dart';

class EarningStatementScreen extends ConsumerWidget {
  const EarningStatementScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    Uint8List? filePdf = ref.watch(filePdfEarningStatementViewProvider);
    final dt = ref.watch(documentEarningStatementDTProvider);
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          FilledButton.icon(
            onPressed: () async {
              var pdfFile = ref.read(filePdfEarningStatementProvider);
