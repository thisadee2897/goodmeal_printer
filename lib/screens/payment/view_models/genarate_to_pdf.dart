...
final genarateToPDFProvider = StateNotifierProvider<GenaratePDFPaymentNotifier, AsyncValue<pw.Document>>(
  (ref) => GenaratePDFPaymentNotifier(ref),
);

class GenaratePDFPaymentNotifier extends StateNotifier<AsyncValue<pw.Document>> {
  GenaratePDFPaymentNotifier(this.ref) : super(AsyncValue.data(pw.Document())) {
  ...
  Future<void> showPrintedPdf() async {
    if (state.hasValue) {
      ...
      ref.read(isLoadGennaratePDFPaymentProvider.notifier).state = false;
    }
  }
}

final isLoadGennaratePDFPaymentProvider = StateProvider<bool>((ref) => false);
